...
from torchvision.models import vit_b_16  # Vision Transformer 모델 로드
...
class EmbeddingNetViT(nn.Module):
    def __init__(self, embedding_dim=128):
        super(EmbeddingNetViT, self).__init__()
        # ViT 모델 로드 (사전 학습된 모델 사용)
        self.vit = vit_b_16(weights=ViT_B_16_Weights.DEFAULT)
        self.vit.heads = nn.Identity()  # 분류 헤드를 제거하여 feature extractor로 사용

        # Fully connected layers to project to embedding_dim
        self.fc = nn.Sequential(
            nn.Linear(768, embedding_dim),  # ViT base outputs 768-dim feature vectors
            nn.ReLU(),
            nn.Linear(embedding_dim, embedding_dim)
        )

    def forward(self, x):
        # ViT expects input of shape [batch_size, channels, height, width]
        x = reshape_input(x)  # 3D -> 4D 변환
        x = self.vit(x)  # ViT로부터 특성 추출
        x = self.fc(x)   # Fully connected layers로 임베딩 생성
        x = F.normalize(x, p=2, dim=1)  # L2 정규화
        return x

def reshape_input(x):
    """
    3D 데이터를 4D로 변환하여 ViT 모델에 맞게 조정
    Args:
        x (torch.Tensor): 입력 데이터 (batch_size, features, sequence_length)
    Returns:
        torch.Tensor: 변환된 데이터 (batch_size, channels, height, width)
    """
    if len(x.size()) == 3:  # 입력 데이터가 (batch_size, features, seq_len)
        batch_size, features, seq_len = x.size()
    else:
        raise ValueError(f"Unexpected input shape: {x.size()}. Expected 3D tensor.")

    # 정사각형 높이와 너비 계산
    height = int(seq_len ** 0.5)
    width = height
    if height * width != seq_len:
        raise ValueError("Sequence length must be a perfect square to reshape into height and width.")

    # 데이터 재구성
    x = x.reshape(batch_size, features, height, width)

    # 채널 수를 3으로 조정 (예: 평균을 사용하거나 반복 복사)
    x = torch.mean(x, dim=1, keepdim=True)  # 채널 평균화
    x = x.repeat(1, 3, 1, 1)  # 1채널 데이터를 3채널로 확장
    return x
...
def load_model(model_path, input_channels=1, embedding_dim=128):
    model = EmbeddingNetViT(embedding_dim=128)
...
transform_vit = transforms.Compose([
    transforms.Resize((224, 224)),  # ViT 입력 크기에 맞춤
    transforms.Grayscale(num_output_channels=3),  # 1채널 데이터를 3채널로 변환
    transforms.ToTensor(),
...
def main():
    ...
    train_dataset = ContrastiveDataset(
        ...
        input_channel=1,    # ViT 입력은 일반적으로 3채널 이미지를 사용하지만, 1채널이라면 그대로 유지
        augment=augment,
    ...
    test_easy_dataset = ContrastiveDataset(
        ...
        labels=test_labels,
        transform=transform_vit,  # ViT용 transform으로 수정
    ...
    def collate_fn(batch):
        """
        DataLoader에서 배치 데이터를 처리하는 함수. 데이터를 패딩하여 동일한 크기로 맞춤.
        Args:
            batch: 각 샘플의 (data1, data2, labels)로 구성된 리스트
        Returns:
            배치 데이터 (data1, data2, labels) 형태로 반환
        """
        data1_list, data2_list, labels_list = zip(*batch)

        # 데이터를 224x224로 패딩
        max_len = max(data.shape[-1] for data in data1_list + data2_list)
        target_length = max(224 * 224, max_len)
        data1_padded = [F.pad(data, (0, target_length - data.shape[-1])) if data.shape[-1] < target_length else data[:, :, :target_length] for data in data1_list]
        data2_padded = [F.pad(data, (0, target_length - data.shape[-1])) if data.shape[-1] < target_length else data[:, :, :target_length] for data in data2_list]

        # 데이터 재구성
        data1_stacked = torch.stack(data1_padded)
        data2_stacked = torch.stack(data2_padded)
        labels_stacked = torch.tensor(labels_list, dtype=torch.float32)

        return data1_stacked, data2_stacked, labels_stacked
    ...
    model = EmbeddingNetViT(embedding_dim=128)
    train_model(model, train_loader=train_loader, num_epochs=args.num_epochs, learning_rate=1e-4, margin=args.margin)
